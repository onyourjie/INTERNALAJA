...
// testwebpanit/app/api/panitiapeserta/import/route.ts
...
// Capitalize setiap kata (Title Case) - SAME AS ROUTE 1
...
// POST - Import CSV dengan kapitalisasi OTOMATIS dan validasi lengkap
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Validasi required fields (tanpa fakultas)
    ...
    // Check untuk duplicate NIMs dalam CSV (setelah di-capitalize)
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      for (const [batchIndex, row] of batch.entries()) {
        ...
        try {
          ...
          // Clean dan CAPITALIZE data OTOMATIS - INI YANG PENTING!
          const cleanData = {
          ...
          // Check if NIM already exists in database (dengan NIM yang sudah di-capitalize)
          ...
          // Generate QR Code dengan data yang sudah di-capitalize
          ...
          // Insert to database dengan data yang sudah di-CAPITALIZE (tanpa fakultas)
          await db.execute(
    ...
    return NextResponse.json({
      success: true,
      message: `Import selesai dalam ${(processingTime / 1000).toFixed(1)}s. ${successData.length} berhasil, ${errorData.length} gagal. Semua data otomatis diformat dengan kapitalisasi yang benar.`,
      data: {
        success: successData,
        errors: errorData,
